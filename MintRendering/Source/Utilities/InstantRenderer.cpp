...
namespace mint
{
	namespace Rendering
	{
		...
		void InstantRenderer::Render() noexcept
		{
			...
			shaderPipelinePool.GetShaderPipeline(_graphicsDevice.GetShaderPipelineLineID()).BindShaderPipeline();
			_lowLevelRendererLine.Render(_graphicsDevice, RenderingPrimitive::LineList);
			_lowLevelRendererLine.Flush();

			shaderPipelinePool.GetShaderPipeline(_graphicsDevice.GetShaderPipelineTriangleID()).BindShaderPipeline();
