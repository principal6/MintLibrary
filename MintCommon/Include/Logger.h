...
#pragma region Logging
    ...
    #define MINT_LOG_UNTAGGED(author, format, ...)              Logger::getInstance().log(nullptr, author, nullptr, nullptr, 0, format, __VA_ARGS__)
    #define MINT_LOG(author, format, ...)                       Logger::getInstance().log(" _LOG_ ", author, __func__, __FILE__, __LINE__, format, __VA_ARGS__)
    #define MINT_LOG_ALERT(author, format, ...)                 Logger::getInstance().logAlert(" ALERT ", author, __func__, __FILE__, __LINE__, format, __VA_ARGS__)
    #define MINT_LOG_ERROR(author, format, ...)                 Logger::getInstance().logError(" ERROR ", author, __func__, __FILE__, __LINE__, format, __VA_ARGS__); _MINT_LOG_ERROR_ACTION
#pragma endregion


#pragma region Assertion
    #define MINT_NEVER                                          { Logger::getInstance().logError(" ASSUR ", "XXX", __func__, __FILE__, __LINE__, "THIS BRANCH IS NOT ALLOWED!"); _MINT_LOG_ERROR_ACTION; }
    // [DESCRIPTION]
    // Return false if expression is false!
    #define MINT_ASSURE(expression)                             if (!(expression)) { Logger::getInstance().logError(" ASSUR ", "XXX", __func__, __FILE__, __LINE__, "NOT ASSURED. RETURN FALSE!"); _MINT_LOG_ERROR_ACTION; return false; }

#if defined MINT_LOG_FOR_ASSURE_SILENT
    #define MINT_ASSURE_SILENT(expression)                      if (!(expression)) { Logger::getInstance().log(" ASSUR ", "XXX", __func__, __FILE__, __LINE__, "NOT ASSURED. RETURN FALSE!"); return false; }
#else
    #define MINT_ASSURE_SILENT(expression)                      if (!(expression)) { return false; }
#endif

    #if defined MINT_DEBUG
        #define MINT_ASSERT(author, expression, format, ...)    if (!(expression)) { Logger::getInstance().logError(" ASSRT ", author, __func__, __FILE__, __LINE__, format, __VA_ARGS__); _MINT_LOG_ERROR_ACTION; }
    #else
