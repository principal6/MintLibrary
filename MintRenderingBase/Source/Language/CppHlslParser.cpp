...
namespace mint
{
    namespace Language
    {
        ...
        const bool CppHlslParser::parseCode(const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& namespaceNode, uint32& outAdvanceCount)
        {
            ...
            {
                MINT_RETURN_FALSE_IF_NOT(parseClassStruct(false, symbolPosition, namespaceNode, outAdvanceCount) == true);
            ...
            {
                MINT_RETURN_FALSE_IF_NOT(parseClassStruct(true, symbolPosition, namespaceNode, outAdvanceCount) == true);
            ...
            {
                MINT_RETURN_FALSE_IF_NOT(parseUsing(symbolPosition, namespaceNode, outAdvanceCount) == true);
                return true;
            }
            else if (fisrtSymbol._symbolString == "namespace")
            {
                MINT_RETURN_FALSE_IF_NOT(parseNamespace(symbolPosition, namespaceNode, outAdvanceCount) == true);
                return true;
            }
            else
            {
        ...
        void CppHlslParser::generateTypeInfo(const TreeNodeAccessor<SyntaxTreeItem>& namespaceNode, const TreeNodeAccessor<SyntaxTreeItem>& classStructNode)
        {
            ...
            if (typeSize == 0)
            {
                MINT_LOG_ERROR("김장원", "typeSize 가 0 입니다!!!");
                return;
            }
        ...
        const bool CppHlslParser::parseClassStruct(const bool isStruct, const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& namespaceNode, uint32& outAdvanceCount)
        {
            ...
            uint32 identifierOffset = 1;
            {
                ...
                if (symbol._symbolClassifier != SymbolClassifier::Identifier)
                {
                    ...
                    if (symbol._symbolString == "alignas")
                    {
                        ...
                        MINT_RETURN_FALSE_IF_NOT(parseAlignas(symbolPosition + 1, classStructNode, alignasAdvanceCount) == true);
            ...
            
            {
                ...
                else if (openSymbol._symbolClassifier == SymbolClassifier::Grouper_Open)
                {
                    ...
                    {
                        uint32 advanceCount = 0;
                        MINT_RETURN_FALSE_IF_NOT(parseClassStructMember(classIdentifierSymbol, currentPosition, classStructNode, currentAccessModifier, advanceCount, continueParsingMember) == true);
...

            {
                ...
                if (symbol._symbolString == classIdentifierSymbol._symbolString || isDestructor == true)
                {
                    ...
                    if (isDestructor == false)
                    {
                        ...
                        {
                            MINT_RETURN_FALSE_IF_NOT(parseFunctionParameters(isDeclaration, openParenthesisPosition + 1, ctorOrDtorNode) == true);
                        }
                        
                        if (postCloseParenthesisSymbol._symbolString == ":")
                        {
                            uint32 initializeListAdvanceCount = 0;
                            MINT_RETURN_FALSE_IF_NOT(parseClassStructInitializerList(closeParenthesisPosition + 2, ctorOrDtorNode, initializeListAdvanceCount) == true);
...

                    {
                        ...
                        else if (symbol._symbolString == "{")
                        {
                            ...
                            MINT_RETURN_FALSE_IF_NOT(parseFunctionInstructions(declarationEndPosition + 1, ctorOrDtorNode, postInstructionAdvance) == true);
                ...
                else
                {
                    ...
                    if (isTypeChunk(currentPosition, postTypeChunkPosition) == true)
                    {
                        ...
                        if (postPostTpeChunkSymbol._symbolString == "(")
                        {
                            ...
                            {
                                MINT_RETURN_FALSE_IF_NOT(parseFunctionParameters(false, openParenthesisPosition + 1, memberFunctionNode) == true);

                                uint32 postInstructionAdvance = 0;
                                MINT_RETURN_FALSE_IF_NOT(parseFunctionInstructions(postAttributePosition + 1, memberFunctionNode, postInstructionAdvance) == true);
                        ...
                        else
                        {
                            ...
                            MINT_RETURN_FALSE_IF_NOT(parseTypeNode(CppHlslTypeNodeParsingMethod::ClassStructMember, currentPosition, memberVariableListNode, typeNode, postTypeNodeOffset) == true);
        ...
        const bool CppHlslParser::parseClassStructInitializerList(const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& ancestorNode, uint32& outAdvanceCount)
        {
            ...
            {
                uint32 advanceCount = 0;
                MINT_RETURN_FALSE_IF_NOT(parseClassStructInitializerList_Item(symbolPosition + currentOffset, initializerListNode, advanceCount, continueParsing) == true);
        ...
        const bool CppHlslParser::parseFunctionParameters(const bool isDeclaration, const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& ancestorNode)
        {
            ...
            do
            {
                MINT_RETURN_FALSE_IF_NOT(parseFunctionParameters_Item(isDeclaration, symbolPosition + currentItemOffset, parameterListNode, advanceCount) == true);
        ...
        const bool CppHlslParser::parseFunctionParameters_Item(const bool isDeclaration, const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& ancestorNode, uint32& outAdvanceCount)
        {
            ...
            TreeNodeAccessor<SyntaxTreeItem> typeNode;
            uint32 postTypeNodeOffset = 0;
            MINT_RETURN_FALSE_IF_NOT(parseTypeNode(CppHlslTypeNodeParsingMethod::FunctionParameter, symbolPosition, ancestorNode, typeNode, postTypeNodeOffset) == true);
        ...
        const bool CppHlslParser::parseFunctionInstructions(const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& ancestorNode, uint32& outAdvanceCount)
        {
            ...
            MINT_RETURN_FALSE_IF_NOT(parseExpression(symbolPosition, instructionListNode, outAdvanceCount));
            ++outAdvanceCount;
        ...
        const bool CppHlslParser::parseExpression(const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& ancestorNode, uint32& outAdvanceCount)
        {
            ...
            if (firstSymbol._symbolString == "(")
            {
                ...
                {
                    uint32 advanceCount = 0;
                    MINT_RETURN_FALSE_IF_NOT(parseExpression(symbolIter, ancestorNode, advanceCount) == true);
            ...
            else
            {
                ...
                if (isTypeChunk(symbolPosition, postTypeChunkPosition) == true)
                {
                    ...
                    uint32 postTypeNodeOffset = 0;
                    TreeNodeAccessor<SyntaxTreeItem> typeNode;
                    MINT_RETURN_FALSE_IF_NOT(parseTypeNode(CppHlslTypeNodeParsingMethod::Expression, symbolPosition, ancestorNode, typeNode, postTypeNodeOffset) == true);
        ...
        const bool CppHlslParser::parseTypeNode(const CppHlslTypeNodeParsingMethod parsingMethod, const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& ancestorNode, TreeNodeAccessor<SyntaxTreeItem>& outTypeNode, uint32& outAdvanceCount)
        {
            ...
            */
            ...
            MINT_RETURN_FALSE_IF_NOT(parseTypeNode_CheckModifiers(parsingMethod, symbolPosition, typeModifierSet, postPremodifierOffset));
            ...
            {
                uint32 advanceCount = 0;
                MINT_RETURN_FALSE_IF_NOT(parseTypeNode_CheckModifiers(parsingMethod, symbolPosition + postPostmodifierOffset, typeModifierSet, advanceCount));
        ...
        {
            MINT_RETURN_FALSE_IF_NOT(hasSymbol(symbolPosition + 4) == true);
            ...
            TreeNodeAccessor<SyntaxTreeItem> typeNode;
            uint32 postTypeNodeOffset = 0;
            MINT_RETURN_FALSE_IF_NOT(parseTypeNode(CppHlslTypeNodeParsingMethod::FunctionParameter, symbolPosition + 3, usingNode, typeNode, postTypeNodeOffset) == true);

            const uint32 typeIndex = registerType(namespaceNode, CppHlslTypeTableItem(typeNode.getNodeData()));
            MINT_RETURN_FALSE_IF_NOT(registerTypeAlias(aliasSymbol._symbolString, typeIndex) == true);
        ...
        const bool CppHlslParser::parseNamespace(const uint32 symbolPosition, TreeNodeAccessor<SyntaxTreeItem>& namespaceNode, uint32& outAdvanceCount)
        {
            ...
            MINT_RETURN_FALSE_IF_NOT(hasSymbol(symbolPosition + 3) == true);
            ...
            {
                uint32 advanceCount = 0;
                MINT_RETURN_FALSE_IF_NOT(parseCode(symbolIter, newNamespaceNode, advanceCount) == true);
                symbolIter += advanceCount;
            }
        ...
        TreeNodeAccessor<SyntaxTreeItem> CppHlslParser::findNamespaceNode(const std::string& namespaceFullIdentifier) const noexcept
        {
            ...
            mint::StringUtil::tokenize(namespaceFullIdentifier, "::", namespaceStack);
        ...
        {
            mint::Vector<std::string> typeStack;
            mint::StringUtil::tokenize(typeFullIdentifier, "::", typeStack);
        ...
        {
            MINT_ASSERT("김장원", isNamespaceNode(namespaceNode) == true, "namespaceNode 가 잘못됐습니다!");
