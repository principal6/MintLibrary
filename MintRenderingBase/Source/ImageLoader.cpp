	...
	namespace Rendering
	{
		...
		bool ImageLoader::loadImage(const Vector<byte>& bytes, ColorImage& outColorImage) const
		{
			...
			for (int32 at = 0; at < dimension; ++at)
			{
				if (at % comp == 0)
				{
					outColorImage.setPixel(at, Color(rawImageData[at], rawImageData[at + 1], rawImageData[at + 2], rawImageData[at + 3]));
				}
			}
		...
		bool ImageLoader::loadImage(const uint32 requiredComponents, const Vector<byte>& inCompressedBytes, Vector<byte>& outDecompressedBytes) const
		{
			...
			int32 height{};
			int32 comp{};
			int32 req_comp{ static_cast<int32>(requiredComponents) };
		...
		bool ImageLoader::saveImagePNG(const StringReferenceA& fileName, const Vector<byte>& decompressedBytes, const uint32 width, const uint32 height, const uint32 components) const
		{
			...
			if (saveImagePNG(decompressedBytes, width, height, components, binaryFileWriter) == true)
			{
		...
		bool ImageLoader::saveImagePNG(const Vector<byte>& decompressedBytes, const uint32 width, const uint32 height, const uint32 components, BinaryFileWriter& binaryFileWriter) const
		{
			int32 length{ 0 };
